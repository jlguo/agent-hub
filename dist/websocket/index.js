...
export function setupWebSocket(io) {
    io.on('connection', (socket) => {
        ...
        // Start a discussion
        socket.on('discussion:start', async (data, callback) => {
            try {
                const { roomId, topic } = data;
                if (!topic) {
                    if (callback)
                        callback({ error: 'Topic is required' });
                    return;
                }
                // Verify room exists
                const room = await prisma.room.findUnique({
                ...
                if (callback)
                    callback({ success: true, discussion });
            }
            catch (error) {
        ...
        socket.on('discussion:updateHeat', async (data, callback) => {
            try {
        ...
        socket.on('agent:action', async (data, callback) => {
            try {
                ...
                if (!agent.isActive) {
                    if (callback)
                        callback({ error: 'Agent is not active' });
                    return;
                }
 ...
 */
export function emitSessionStatus(io, roomId, status) {
