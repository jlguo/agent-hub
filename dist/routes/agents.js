...
router.post('/rooms/:roomId', async (req, res) => {
    try {
        const { roomId } = req.params;
        const { name, role, avatar, talkativeness = 50, empathy = 50, curiosity = 50, systemPrompt, responseDelay = 1000, } = req.body;
        if (!name || !role) {
...
router.get('/rooms/:roomId/:id', async (req, res) => {
    try {
...
router.put('/rooms/:roomId/:id', async (req, res) => {
    try {
        const { roomId, id } = req.params;
        const { name, role, avatar, talkativeness, empathy, curiosity, systemPrompt, isActive, responseDelay, } = req.body;
...
router.delete('/rooms/:roomId/:id', async (req, res) => {
    try {
        const { roomId, id } = req.params;
...
router.post('/rooms/:roomId/agents/:agentId/relationships', async (req, res) => {
    try {
