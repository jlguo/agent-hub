import { AppError, ErrorType, errors, safeExecute, formatErrorResponse, logError } from '../error-handler';
...
describe('safeExecute', () => {
  ...
  it('should throw AppError on failure', async () => {
    ...
    await expect(safeExecute(operation, 'Operation failed', { userId: '123' }))
      .rejects
      .toThrow(AppError);
  ...
  it('should rethrow AppError without wrapping', async () => {
    ...
    await expect(safeExecute(operation, 'Wrapper message'))
      .rejects
      .toBe(appError);
  });
});
...
describe('logError', () => {
  it('should log error with context', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('[ErrorHandler]', expect.objectContaining({
      service: 'MessageService',
      operation: 'sendMessage',
